...
BEGIN
  ...
  -- Notify admin via edge function (wrapped in exception handler so it never
  -- blocks profile/library creation if pg_net is unavailable or the call fails)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Log but do not rethrow — profile and library creation must always succeed
    RAISE WARNING 'notify-new-user call failed for user %: %', NEW.id, SQLERRM;
  END;
